...
# Macro for generating/modifying some diagnostic info in a status indicator file
BSTAMP=-e 'stamp_build(target="$@", dependencies=$(call rvector,$^))'
...
% : %.s3 \
		lib/src/status.R lib/src/s3.R lib/cfg/s3_config.yaml lib/cfg/s3_post.yaml
	@if [ $@ -ot $@.s3 -o ! -e $@ ]; then\
		${RSCRIPT}\
		-e 'get_s3(file.name="$@", s3.config="lib/cfg/s3_config.yaml")'\
		-e 'warnings()' -e 'devtools::session_info()' > $(dir $(patsubst %/,%,$(dir $@)))log/$(notdir $(basename $@)).s3.log 2>&1;\
	else\
...
% : %.loc \
		lib/src/status.R
	@if [ $@ -ot $@.loc -o ! -e $@ ]; then\
		echo "$@ is out of date or locally unavailable";\
	else\
		echo "$@ is available locally";\
	fi;
